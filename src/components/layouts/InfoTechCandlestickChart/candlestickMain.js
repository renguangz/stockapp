...
const CandlestickMain = ({ price, fetchPrice }) => {
    ...
    const svgRef = useRef();
    const mainSvgContainerRef = useRef();

    const svg = d3.select(svgRef.current)
    const svgContainer = d3.select('.mainSvg')
    // d3.select('.candlestickMainChart').remove()
    // const svg = svgContainer.append('svg').attr('class', 'candlestickMainCahrt')
    const width = 990
    const height = 275
    ...
    useEffect(() => {
        d3.select('.candlestickMainChart').remove()
        const width = mainSvgContainerRef.current.offsetWidth // 990
        const height = mainSvgContainerRef.current.offsetHeight
        const box = d3.select(mainSvgContainerRef.current)
        const svg = box.append('svg').attr('class', 'candlestickMainChart').attr('width', width).attr('height', height)
        const margin = { top: 0, left: 10, bottom: 30, right: 70 }
        const innerWidth = width - margin.left - margin.right
        const innerHeight = height - margin.top - margin.bottom
    ...
    return (
        <>
            ...
            <TechMainContainer height={'40'}>
                ...
                <TechMainSvg className='mainSvg' ref={mainSvgContainerRef}>
                    <Svg ref={svgRef} className='candlestickMainChart' />
                </TechMainSvg>
            </TechMainContainer>
        </>
    )
